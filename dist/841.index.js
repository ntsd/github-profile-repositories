...
class Body {
	...
	async formData() {
		...
		const {toFormData} = await __webpack_require__.e(/* import() */ 905).then(__webpack_require__.bind(__webpack_require__, 905));
...
const extractContentType = (body, request) => {
	...
	if (Buffer.isBuffer(body) || external_node_util_.types.isAnyArrayBuffer(body) || ArrayBuffer.isView(body)) {
		return null;
	}
...
async function fetch(url, options_) {
	return new Promise((resolve, reject) => {
		...
		request_.on('response', response_ => {
			...
			let body = (0,external_node_stream_.pipeline)(response_, new external_node_stream_.PassThrough(), reject);
			...
			if (codings === 'gzip' || codings === 'x-gzip') {
				body = (0,external_node_stream_.pipeline)(body, external_node_zlib_.createGunzip(zlibOptions), reject);
				response = new Response(body, responseOptions);
				resolve(response);
				return;
			}
			...
			if (codings === 'deflate' || codings === 'x-deflate') {
				...
				const raw = (0,external_node_stream_.pipeline)(response_, new external_node_stream_.PassThrough(), reject);
				raw.once('data', chunk => {
					// See http://stackoverflow.com/questions/37519828
					body = (chunk[0] & 0x0F) === 0x08 ? (0,external_node_stream_.pipeline)(body, external_node_zlib_.createInflate(), reject) : (0,external_node_stream_.pipeline)(body, external_node_zlib_.createInflateRaw(), reject);
			...
			// For br
			if (codings === 'br') {
				body = (0,external_node_stream_.pipeline)(body, external_node_zlib_.createBrotliDecompress(), reject);
				response = new Response(body, responseOptions);
				resolve(response);
				return;
			}
